...
func runGoEntrypoint(args []string, _ func(string)) error {
	// Go doesn't need version management - uses system Go
	// Just exec the command
	return execCommand(args)
}

func runRustEntrypoint(args []string, _ func(string)) error {
	home := os.Getenv("HOME")
...
func runAndroidEntrypoint(args []string, _ func(string)) error {
	home := os.Getenv("HOME")
	...
	if androidHome == "" {
		...
		_ = os.Setenv("ANDROID_HOME", androidHome)
	}
...
func runGenericEntrypoint(args []string, _ func(string)) error {
