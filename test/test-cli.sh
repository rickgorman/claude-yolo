...
section "extract_git_user_config — multiple includes with override"

GIT_CONFIG_MULTI="$TMPDIR_BASE/git-config-multi"
mkdir -p "$GIT_CONFIG_MULTI/.config"
cat > "$GIT_CONFIG_MULTI/.gitconfig" << 'EOF'
[include]
	path = ~/.config/git-base
	path = ~/.config/git-override
EOF
cat > "$GIT_CONFIG_MULTI/.config/git-base" << 'EOF'
[user]
	name = Base User
	email = base@example.com
EOF
cat > "$GIT_CONFIG_MULTI/.config/git-override" << 'EOF'
[user]
	name = Override User
EOF
result=$(HOME="$GIT_CONFIG_MULTI" extract_git_user_config)
assert_contains "Multi include: uses last name" "$result" "user.name=Override User"
assert_contains "Multi include: keeps first email" "$result" "user.email=base@example.com"
